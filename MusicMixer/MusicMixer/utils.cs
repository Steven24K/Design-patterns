...
namespace MusicMixer
{
    ...
    public interface ISoundManager
    {
        ...
        void DeleteLastNote();
        int AmountOfNotes { get; }
        Note[] Song { get; }
        Iterator<Note> GetSongList{get;}
    }
    ...
    public class ArrayItterator<T> : Iterator<T>
    {
        ...
        public void DeleteLast()
        {
            if (AmountOfItems > 0)
            {
                this._Array[AmountOfItems - 1] = default(T);
                AmountOfItems -= 1;
            }
        }
        ...
        public IOption<T> GetLast()
        {
            if (AmountOfItems > 0) return new Some<T>(this._Array[AmountOfItems-1]);
            return new None<T>();
        }
        ...
        public override IOption<IComponent> Create(int id, Position top_left_corner, Action action)
        {
            ...
            switch (id)
            {
                ...
                case 1: //Creates a play song button
                    return new Some<IComponent>(new MonoButton(top_left_corner, new Colour(0, 255, 50), 100, 50, action));
                case 2: //creates a note on the screen
                    return new Some<IComponent>(new DrawableNote(top_left_corner));
                case 3: //Create delete button
                    return new Some<IComponent>(new MonoButton(top_left_corner,new Colour(255,0,0),100,50,action));
                default:
